"""JSON Schema helper types for tool parameter definitions."""
...
from agentic_mm_rag.tools.base import Schema
...
class ObjectSchema(Schema):
    ...
    def to_json_schema(self) -> dict[str, Any]:
        schema_type: Any = "object"
        if self._nullable:
            schema_type = ["object", "null"]
        out: dict[str, Any] = {
            "type": schema_type,
            "properties": {
                key: Schema.fragment(value) for key, value in self._properties.items()
            },
        }
        if self._required:
